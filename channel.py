...
class channel(object):
    ...
    # def construct_RIS_phase_precoder_combiner(self, theta0):
    #     """
    ...
    #     omega_tilde = self.RIS_random_codebook() # size=(T/2, N_R)
    #     # omega_tilde = self.RIS_directional()
    #     # self.Omega_tilde = np.zeros((int(self.T/2), np.prod(self.N_R), np.prod(self.N_R)), dtype=np.complex128)
    #     # self.Omega_tilde[:, np.arange(np.prod(self.N_R)), np.arange(np.prod(self.N_R))] = self.omega_tilde # Diagonalize; size=(T/2, N_R, N_R)

    #     # self.omega = self.orthogonal_RIS(self.omega_tilde) # size=(T, N_R)
    #     # self.Omega = np.zeros((self.T, np.prod(self.N_R), np.prod(self.N_R)), dtype=np.complex128)
    #     # self.Omega[:, np.arange(np.prod(self.N_R)), np.arange(np.prod(self.N_R))] = self.omega # Diagonalize; size=(T, N_R, N_R)
...
"""
    ...
    # def simulate_node_orientations(self, num):
    #     """
    #     Simulate orientation of num nodes uniformly in [-\pi, \pi]\times[0, \pi/2]
    #     """
    #     o = np.zeros((num, 2))
    #     o[:, 0] = np.random.uniform(-np.pi, np.pi)
    #     o[:, 1] = np.random.uniform(0, np.pi/2)
    #     return o
    ...
    def angles(self, p, p_U, o_U):
        ...
        # N_el, N_az = 4, 4
        # pos_local = np.stack(((N_el+1)/2 - np.tile(np.arange(1, N_el+1), N_az),
        #                         (N_az+1)/2 - np.repeat(np.arange(1, N_az+1), N_el),
        #                         np.ones(N_az*N_el)), axis=-1) * 0.002
        # pos_global = np.einsum("ij,ni->nj", O_U[0, 0, :, :], pos_local) # size=(N_az*N_el, 3)
        ...
        #     ax.set_ylabel("y")
        #     ax.set_zlabel("z")
        #     ax.axes.set_xlim3d(left=-0.01, right=0.01) 
        #     ax.axes.set_ylim3d(bottom=-0.01, top=0.01) 
        #     ax.axes.set_zlim3d(bottom=-0.01, top=0.01) 
        #     plt.legend()
        #     plt.show()
    ...
    def coord_rotation_matrix(self, yaw=np.pi/2, pitch=0, roll=0):
        ...
        O = -(Rz @ Ry @ Rx)
        # O = np.array([[np.cos(azimuth), -np.sin(azimuth)*np.cos(elevation), -np.sin(azimuth)*np.sin(elevation)],
        #               [np.sin(azimuth), np.cos(azimuth)*np.cos(elevation), np.cos(azimuth)*np.sin(elevation)],
    ...
    # def ULA_array_response_vector(self, phi, N_Q, d, func):
    #     """
    #     Compute the array response vector of a uniform linear array at the given
    #     angle phi (size=(K,) or (K, L) or (K, E, L)) for a given number of
    #     antennas N_Q spaced by distance d.
    #     The trigonometric function is specified by func.
    #     For azimuth func is np.cos and for elevation func is np.sin.
    #     """
    #     if phi.ndim == 1:
    #         idx = np.arange(N_Q) # size=(N_Q,)
    #         gamma = np.exp(1j*2*np.pi*func(phi[:, None])*d/self.lambda_)**idx[None, :] # size=(K, N_Q)
    #     elif phi.ndim == 2:
    #         idx = np.arange(N_Q) # size=(N_Q,)
    #         gamma = np.exp(1j*2*np.pi*func(phi[:, :, None])*d/self.lambda_)**idx[None, None, :] # size=(K, L, N_Q)
    #     elif phi.ndim == 3:
    #         idx = np.arange(N_Q) # size=(N_Q,)
    #         gamma = np.exp(1j*2*np.pi*func(phi[:, :, :, None])*d/self.lambda_)**idx[None, None, None, :] # size=(K, E, L, N_Q)
    #     else:
    #         raise ValueError("Input must have 1 or 2 or 3 dimensions.")
    #     return gamma
    ...
    # def AntenPOS(self, N_az, N_el, d):
    #     """
    #     Generating uniform plarnar antenna (UPA) antenna positions.
    #     UPA lies on x-z plane, not x-y plane. Antenna is heading to y.
    #     Azimuth from +x axis (counter-clock); elevation from +z axis to down
    #     """
    #     # KE = az.shape[0]
    #     # R = np.zeros((KE, 3, 3))
    #     # for k in range(KE):
    #     #     R[k, :, :] = self.coord_rotation_matrix(az[k], el[k]) # fix? size=(K*E, 3, 3)

    #     pos_local = np.stack(((N_el+1)/2 - np.tile(np.arange(1, N_el+1), N_az),
    #                            np.zeros(N_az*N_el),
    #                            (N_az+1)/2 - np.repeat(np.arange(1, N_az+1), N_el)), axis=-1) * d
    #     # pos_global = np.einsum("kij,ni->knj", R, pos_local) # size=(K*E, N_az*N_el, 3)

    #     if N_az == 4 and self.verbose is True:
    #         fig = plt.figure()
    #         ax = fig.add_subplot(projection='3d')
    #         ax.scatter(pos_local[:, 0], pos_local[:, 1], pos_local[:, 2], label="local")
    #         #ax.scatter(pos_global[0, :, 0], pos_global[0, :, 1], pos_global[0, :, 2], label="global")
    #         ax.set_xlabel("x")
    #         ax.set_ylabel("y")
    #         ax.set_zlabel("z")
    #         ax.axes.set_xlim3d(left=-0.01, right=0.01) 
    ...
    # def funcFIM_waveVec(self, az, el):
    #     """
    #     Wavenumber vector.
    #     """
    #     waveVec = 2*np.pi/self.lambda_ * np.stack((np.cos(az)*np.sin(el), np.sin(az)*np.sin(el), np.cos(el)), axis=-1)
    #     return waveVec

    # def UPA_array_response_vector_v2(self, phi_vec, N_Q, d):
    #     """
    #     Uniform planar array response vector from antenna positions in local coordinates.
    #     """

    #     # if phi_vec.ndim == 1:
    #     #     K, E, L = 1, 1, 1
    #     #     az, el = np.array([phi_vec[0]]), np.array([phi_vec[1]])
    #     # elif phi_vec.ndim == 2:
    #     #     K, E, L = 1, 1, phi_vec.shape[0]
    #     #     az, el = phi_vec[:, 0], phi_vec[:, 1]
    #     # elif phi_vec.ndim == 3:
    #     #     K, E, L = phi_vec.shape[0], phi_vec.shape[1], 1
    #     #     az, el = phi_vec[:, :, 0].flatten(), phi_vec[:, :, 1].flatten()
    #     # elif phi_vec.ndim == 4:
    #     #     K, E, L = phi_vec.shape[:-1]
    #     #     az, el = phi_vec[:, :, :, 0].flatten(), phi_vec[:, :, :, 1].flatten()
    #     # else:
    #     #     raise ValueError("Input angles must have 1 or 2 or 3 or 4 dimensions.")

    #     # if o_Q.ndim == 1:
    #     #     assert K == 1 and E == 1, "Dimension mismatch."
    #     #     Q_az, Q_el = np.array([o_Q[0]]), np.array([o_Q[1]])
    #     # # elif o_Q.ndim == 2:
    #     # #     assert L == o_Q.shape[0], "Dimension mismatch"
    #     # #     Q_az, Q_az = o_Q[:, 0], o_Q[:, 1]
    #     # elif o_Q.ndim == 3:
    #     #     assert K == o_Q.shape[0] and E == o_Q.shape[1], "Dimension mismatch"
    #     #     Q_az, Q_az = o_Q[:, :, 0].flatten(), o_Q[:, :, 1].flatten()
    #     # # elif o_Q.ndim == 4:
    #     # #     assert K == o_Q.shape[0] and E == o_Q.shape[1] and L == o_Q.shape[2], "Dimension mismatch"
    #     # #     Q_az, Q_az = o_Q[:, :, :, 0].flatten(), o_Q[:, :, :, 1].flatten()
    #     # else:
    #     #     raise ValueError("Input orientations must have 1 or 3 dimensions.")

    #     antPos = self.AntenPOS(N_Q[0], N_Q[1], d) # size=(N_az*N_el, 3)
    #     waveVec = self.funcFIM_waveVec(phi_vec[..., 0], phi_vec[..., 1]) # size=(..., 3)
    #     a = 1/np.sqrt(np.prod(N_Q))*np.exp(-1j * np.einsum("ni,...i->...n", antPos, waveVec)) # size=(..., N_az*N_el)
    #     return a
    ...
    def construct_precoder(self, theta0):
        ...
        # aU_theta0_H_null_space = np.zeros((self.K, self.E, np.prod(self.N_U), np.prod(self.N_U)-1), dtype=np.complex128)
        # for k in range(self.K):
        #     for tau in range(self.E):
        #         aU_theta0_H_null_space[k, tau, :, :] = null_space(aU_theta0[k, tau, None, :]) # size=(K, E, N_U, N_U-1)
        ...
        # choice = np.random.randint(0, np.prod(self.N_U)-1, size=(self.T2))
        # weights = np.zeros((self.T2, np.prod(self.N_U)-1))
        # for idx, s in enumerate(choice):
        #     weights[idx, s] = 1
        # #weights = np.random.uniform(low=0, high=1, size=(self.T2, np.prod(self.N_U)-1))
        # ns_vectors = np.einsum("keij,tj->tkei", aU_theta0_H_null_space, weights) # size=(T2, K, E, N_U)
        # f_tilde[self.T1:, :, :, :] = ns_vectors / np.linalg.norm(ns_vectors, axis=-1)[:, :, :, None] # size=(T2, K, E, N_U)
    ...
    def construct_combiner(self, theta0):
        ...
        # W_entry0 = np.expand_dims(aU_theta0 / np.linalg.norm(aU_theta0, axis=-1)[:, :, None], -1) # size=(K, E, N_U, 1)
        # W_perp = np.zeros((self.K, self.E, np.prod(self.N_U), np.prod(self.N_U)-1), dtype=np.complex128)
        # for k in range(self.K):
        #     for tau in range(self.E):
        #         W_perp[k, tau, :, :] = null_space(np.conjugate(aU_theta0[k, tau, None, :])) # size=(K, E, N_U, N_U-1)
        # W = np.concatenate((W_entry0, W_perp), axis=-1)
    ...
    def plot_beampattern(self, theta0, W_mat, f_tilde):
        ...
        # #elevation = np.array([self.phil[0, 1]])
        # C1, C2 = np.meshgrid(azimuth, elevation)
        # angle_grid = np.stack((C1.flatten(), C2.flatten()), axis = 1)
        # aR_phi0 = self.UPA_array_response_vector(self.phi0, self.N_R, self.d_R) # size=(K, E, N_R)
        # aR_angle = self.UPA_array_response_vector(angle_grid, self.N_R, self.d_R) # size=(L, N_R)
        # amp_RIS = np.sum(np.abs(np.einsum("gi,ti->gt", aR_angle,
        #                     np.einsum("tij,j -> ti", self.Omega, aR_phi0[0, 0, :])))**2, axis=1)
        # fig = plt.figure()
        # ax1 = fig.add_subplot(1, 2, 1, projection='polar')
        # ax1.plot(azimuth, amp_RIS/max(amp_RIS))
        # ax2 = fig.add_subplot(1, 2, 2)
        # ax2.plot(azimuth*180/np.pi, np.array(amp_RIS)/max(amp_RIS))
        # #plt.axvline(x=self.phil[0, 0]*180/np.pi, color="green")
        # #plt.axvline(x=-self.phil[0, 0]*180/np.pi, color="green")
        # plt.title("RIS response")
        # plt.show()
