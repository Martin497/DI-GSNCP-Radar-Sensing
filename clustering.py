...
class sensing_DBSCAN(sensing_oracle, DIGSNCP_base):
    ...
    def __init__(self, rho_interp, Sigma_interp, sensor_resolution_function,
        ...
        rho_interp : fun
            ...
            the related detection probability. The default is None.
        Sigma_interp : fun
            Input function taking a spatial position and returning
    ...
    # def point_process_loss(self, Psi, Phi_est, Phi_E_est, lambda_c_est):
    #     """
    #     Parameters
    #     ----------
    #     Psi : TYPE
    ...
    #     Sigma_interp : TYPE
    #         DESCRIPTION.
    #     sensor_resolution_function : TYPE
    #         DESCRIPTION.
    ...
    #     innovations = np.zeros(self.K)
    #     for k, Psi_k in enumerate(Psi):
    #         I_k = -self.area
    #         for p in Psi_k:
    #             eta_k_p = self.intensity_value(p, Phi_est, rho_Phi[k], Sigma_Phi[k])
    #             Z_Phi_est_Psi = lambda_c_est + np.sum(eta_k_p)
    #             I_k += 1/Z_Phi_est_Psi
    #         innovations[k] = I_k
    #     loss = np.linalg.norm(innovations)
    #     return loss
    ...
    def __call__(self, Psi, eps, min_samples, **kwargs):
        ...
        return Tilde_Phi_est, Sigma_Tilde_Phi_est, rho_est, pars_est
...
def main():
    ...
    Tilde_Phi_DBSCAN, Sigma_est, rho_est, _ = DBSCAN_alg(Psi, min_samples, eps,**call_kwargs)
